...
function showCalculationsDetails(data) {
  ...
  const coefficient_bud_roof =
    Math.round(data.coefficient_bud_roof * 1000) / 1000;
  const coefficient_bud_wall =
    Math.round(data.coefficient_bud_wall * 1000) / 1000;
  ...
  calculationResultsWall.textContent = `${az} <= ${azfWall} = 1.18 × (${kyWall} × ${knWall}) × (${kzab} / ${coefficient_bud_wall}) × ${KNWall} / (${kyWall} + ${knWall})`;

  // Show calculation details for roof
  const calculationResultsRoof = document.getElementById(
    "calculationResultsRoof"
  );
  calculationResultsRoof.textContent = `${az} <= ${azfRoof} = 1.18 × (${kyRoof} × ${knRoof}) × (${kzab} / ${coefficient_bud_roof}) × ${KNRoof} / (${kyRoof} + ${knRoof})`;
